...
<head>
    ...
    <style>
        ...
        .header-profil {
            ...
            margin-bottom: 30px; animation: masuk 0.5s ease both;
        }
        .profil-info { display: flex; align-items: center; gap: 20px; }
        .profil-avatar { font-size: 50px; }
        .profil-teks h1 { font-family: 'Baloo 2', cursive; margin: 0; font-size: 28px; }
        ...
        .papan-grid-size { font-size: 14px; color: var(--abu); margin-bottom: 20px; font-weight: 700; flex: 1; }

        .aksi-papan { display: flex; flex-direction: column; gap: 8px; margin-top: auto; }
        .aksi-baris { display: flex; gap: 8px; }
        .btn-aksi {
        ...
        .btn-cetak:hover { background: #BAE6FD; }
        ...
        .btn-tambah {
            background: var(--gelap); color: white;
        ...
        @media (max-width: 480px) {
            .header-profil { flex-direction: column; text-align: center; gap: 20px; padding: 20px; }
        ...
        .urutan-badge {
            background: #F1F5F9; color: var(--abu);
        ...
        .urutan-kontrol { display: flex; align-items: center; gap: 4px; }
        .btn-panah {
            background: #F1F5F9; border: none; border-radius: 6px;
            padding: 4px 8px; font-size: 11px; cursor: pointer;
            color: var(--abu); transition: 0.2s; font-weight: bold;
        }
        .btn-panah:hover { background: var(--biru); color: white; }

        /* State nonaktif */
        .card-papan.nonaktif { opacity: 0.45; border-color: #E5E7EB; }
        ...
        .section-header h3 {
            ...
            color: var(--abu); margin: 0;
        }
        .section-header p { font-size: 13px; color: var(--abu); font-weight: 600; margin: 4px 0 0 0; }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <!-- Section: Papan Buatan Sendiri -->
    <div class="section-header">
        <div>
            <h3>📁 Papan Buatan Sendiri</h3>
        </div>
    </div>
    <div class="grid-papan" id="grid-papan-milik">
        <?php if (empty($papan_list)): ?>
            <div style="grid-column: 1/-1; text-align: center; padding: 50px;">
                <p style="color: var(--abu); font-weight: 700;">Belum ada papan buatan sendiri.<br>Yuk, buatkan satu untuk <?= htmlspecialchars($profil['nama']) ?>!</p>
            </div>
        <?php else: ?>
            <?php foreach ($papan_list as $p): $global_idx++; ?>
                <div class="card-papan <?= empty($p['is_aktif']) ? 'nonaktif' : '' ?>" id="papan-<?= $p['id'] ?>" data-tipe="milik" data-id="<?= $p['id'] ?>">
                    <div class="toggle-wrapper">
                        ...
                        <div class="urutan-kontrol">
                            <button type="button" class="btn-panah" onclick="geserUrutan(this, 'atas', 'grid-papan-milik', 'milik')" title="Naikkan urutan">▲</button>
                            <span class="urutan-badge">#<?= $global_idx ?></span>
                            <button type="button" class="btn-panah" onclick="geserUrutan(this, 'bawah', 'grid-papan-milik', 'milik')" title="Turunkan urutan">▼</button>
                        </div>
                    </div>
                    ...
                    <div class="aksi-papan">
                        <a href="../papan/index.php?papan_id=<?= $p['id'] ?>" target="_blank" class="btn-aksi btn-preview">👁 Pratinjau</a>
                        <div class="aksi-baris">
                            <a href="papan-cetak.php?id=<?= $p['id'] ?>" target="_blank" class="btn-aksi btn-cetak">🖨 Cetak</a>
                            <a href="papan-edit.php?id=<?= $p['id'] ?>" class="btn-aksi btn-edit">✏️ Atur</a>
                            <a href="papan-hapus.php?id=<?= $p['id'] ?>" class="btn-aksi btn-hapus">🗑 Hapus
    ...
    <div class="section-header" style="margin-top: 36px;">
        <div>
            <h3>🌍 Papan Siap Pakai</h3>
            <p>Aktifkan papan siap pakai yang ingin ditampilkan untuk <?= htmlspecialchars($profil['nama']) ?></p>
        </div>
    </div>
    <div class="grid-papan" id="grid-papan-preset">
        <?php foreach ($preset_list as $pr): $global_idx++; ?>
            <div class="card-papan <?= empty($pr['is_aktif_preset']) ? 'nonaktif' : '' ?>" id="preset-<?= $pr['id'] ?>" data-tipe="preset" data-id="<?= $pr['id'] ?>">
                <div class="toggle-wrapper">
                    ...
                    <div class="urutan-kontrol">
                        <button type="button" class="btn-panah" onclick="geserUrutan(this, 'atas', 'grid-papan-preset', 'preset')" title="Naikkan urutan">▲</button>
                        <span class="urutan-badge">#<?= $global_idx ?></span>
                        <button type="button" class="btn-panah" onclick="geserUrutan(this, 'bawah', 'grid-papan-preset', 'preset')" title="Turunkan urutan">▼</button>
                    </div>
                </div>
                ...
                <div class="aksi-papan">
                    <a href="../papan/index.php?papan_id=<?= $pr['id'] ?>" target="_blank" class="btn-aksi btn-preview" style="width: 100%;">👁 Lihat Pratinjau</a>
                </div>
            </div>
        <?php endforeach; ?>
    </div>
...
function rekalkNomor() {
    let counter = 1;
    ['grid-papan-milik', 'grid-papan-preset'].forEach(gridId => {
        const grid = document.getElementById(gridId);
        if (!grid) return;
        grid.querySelectorAll('.card-papan').forEach(card => {
            const badge = card.querySelector('.urutan-badge');
    ...
    fetch('papan-aksi.php', { method: 'POST', body: formData })
        ...
        .then(data => {
            if (data.status === 'sukses') {
                card.classList.toggle('nonaktif', !data.is_aktif);
                checkbox.parentElement.title = data.is_aktif ? 'Aktif — tampil di layar anak' : 'Nonaktif — disembunyikan';
            } else {
                checkbox.checked = !checkbox.checked;
                alert('Gagal mengubah status papan.');
            }
        })
        .catch(() => { checkbox.checked = !checkbox.checked; alert('Terjadi kesalahan koneksi.'); });
}
...
function geserUrutan(button, arah, gridId, tipe) {
    ...
    cards.forEach((item, index) => {
        dataUrutan.push({ id: item.dataset.id, urutan: index, tipe: tipe });
    });
